(use-package evil-leader ; ----------------------------------- Set your VIM leader key
  ...
  (evil-leader/set-key
    "e" 'counsel-find-file ; --------------------------------- Default file search
    "cff" 'counsel-fzf ; ------------------------------------- Recursive search by file name with fuzzy matching
    ...
    "k" 'kill-buffer ; --------------------------------------- Default kill buffer
    "i" 'swiper ; -------------------------------------------- Isearch current buffer with swiper
    "z" 'shell ; --------------------------------------------- Default open shell buffer
    "s" 'save-buffer)) ; ------------------------------------- Default save current buffer
